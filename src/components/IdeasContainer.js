...
class IdeasContainer extends Component {
  ...
  updateIdea = (idea) => {
    ...
    const notification = `Your idea entitled "${idea.title}" has been successfully updated`
    this.setState({ideas: ideas, notification: notification})
  }
  ...
  deleteIdea = (id) => {
    ...
    .then(response => {
      const ideaIndex = this.state.ideas.findIndex(x =>
        x.id === id)
      const ideas = update(this.state.ideas, {$splice:
        [[ideaIndex, 1]]})
